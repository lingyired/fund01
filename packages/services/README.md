...
| `market.ts` | 指数大盘：`getIndices` / `getIndexHistory`（东方财富 + 腾讯 + 新浪多源） |
| `index.ts` | 统一导出 |
...
| `push2delay.eastmoney.com` | 指数实时 / 黄金(AU9999)指数实时 | 公共参数 `ut=fa5fd1943c7b386f172d6893dbbd4dc`；备用 `push2.eastmoney.com` / `82.push2.eastmoney.com` |
...
```typescript
import { getFundsQuotes, getIndices } from '@fund01/services'

// 批量拉基金行情
const quotes = await getFundsQuotes(holdFunds, 'fund123')

// 拉指数（含黄金 AU9999 / COMEX 黄金指数入口）
...
- **多源 fallback**：指数有主源 + 备用源，主源超时（8 秒）后自动切换备用源
