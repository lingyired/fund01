    ...
    <SectionCard title="数据说明">
      <div className="space-y-3">
        ...
        <DocItem q="估值兜底规则">
          ...
          <p>
            <b className="text-ink">QDII</b>：无重仓股也无 fund123 分时估值，盘中与未更新期间当日收益均显示「-」（灰色）；<b className="text-ink">「披露日窗口」判定今日是否已更新</b>（QDII 披露日 = 净值日的下一交易日，T+1）——今天披露的净值（净值日=昨天）才显示当日收益，昨天披露的（净值日=前天）保持「-」，避免把未更新的滞后涨幅累计到「当日」标签下。净值日期恒标注在基金名下。官方净值更新后显示「已更新」徽标（与当日收益同步，披露日当天保留，次日/周末无新披露自动清除）。
          </p>
          <p>
