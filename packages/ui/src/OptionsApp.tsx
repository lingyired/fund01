    ...
    <SectionCard title="个人设置">
      ...
      <div className="space-y-2 border-t border-line/50 pt-3">
        ...
        <div className="space-y-1 pt-1">
          ...
          <p className="text-[11px] text-muted">
            估值兜底规则：FundMNFInfo 盘中无估值时，先用重仓股当日涨跌幅自算；自算失败时，非 QDII 基金（黄金/商品 ETF 联接等）自动改用该基金的 fund123 官方分时估值，QDII 基金则保持现状（fund123 无 QDII 当日估值，等 T+1 净值披露，避免把昨日涨幅当今日）；兜底仍失败则保持现状，晚间官方净值披露后自动显示当日收益。
          </p>
        </div>
      </div>
