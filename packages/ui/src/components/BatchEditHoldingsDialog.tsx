...
export function BatchEditHoldingsDialog({
  ...
  const tabs: {id: string; label: string}[] = useMemo(() => {
    const t: {id: string; label: string}[] = [{id: 'all', label: '全部'}]
    for (const g of groups) {
      t.push({id: g || '__ungrouped__', label: g || '未分组'})
    }
    return t
  }, [groups])
  ...
  const activeGroupKey =
    activeTab === 'all' ? null : activeTab === '__ungrouped__' ? '' : activeTab
  ...
  return (
    <Dialog.Root open={open} onOpenChange={(v) => !saving && onOpenChange(v)}>
      <Dialog.Content className="rt-popup-dialog max-w-3xl">
        ...
        {/* 分组 Tab 栏 */}
        {tabs.length > 1 ? (
          <div className="shrink-0 flex gap-1 overflow-x-auto border-b border-line/70">
            {tabs.map((t) => {
              ...
              return (
                <button
                  ...
                  disabled={saving}
                  className={`relative whitespace-nowrap px-3 py-2 text-sm transition-colors disabled:opacity-50 ${
                    active
        ...
        <div className="min-h-0 flex-1 overflow-auto pr-1">
          {visibleItems.length === 0 ? (
            ...
            <table className="w-full table-fixed text-left text-xs">
              ...
              <tbody>
                {visibleItems.map(({r, i}, idx) => {
                  // 全部 tab 下，仅当该基金在多分组时禁用排序（避免跨分组交换）
                  const canMove = !isAllTab
