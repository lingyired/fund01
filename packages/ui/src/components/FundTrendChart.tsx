...
/**
 * 基金走势图（不含 Dialog 外壳），可独立嵌在任意位置（详情弹窗底部 / 独立走势弹窗内）。
 * - active：是否处于可视/激活状态；仅当为 true 时才拉取与渲染曲线，避免弹窗未打开时浪费请求。
 * - 数据加载与 FundTrendDialog 保持一致：分时预加载点优先，否则按需懒加载；历史区间并行预取。
 */
export function FundTrendChart({
  ...
  useEffect(() => {
    ...
    const apply = () => setChartHeight(chartHeightProp ?? (mq.matches ? 300 : 220))
    apply()
  ...
  }, [active, code, fundKey, intradayPoints, lazyIntraday.length])
  ...
  useEffect(() => {
    if (!active || !code || range !== 'since') return
  ...
  }, [active, code, range, byRange.since])
