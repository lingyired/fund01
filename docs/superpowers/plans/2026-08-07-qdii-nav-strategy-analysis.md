...
| **100050** 富国全球债券(QDII)A | **08-06（18:14 前后披露）** | **-0.31%**（当日收益） | 已更新 | ✅ 披露日窗口自动切换生效 |
| **022184** 富国全球科技互联网股票(QDII)C | **08-06（18:14 前后披露）** | **-2.89%**（当日收益） | 已更新 | ✅ 披露日窗口自动切换生效 |

### 11.2 待观察事项（100050 / 022184）

QDII 净值是**逐个/分批披露**的（同一批基金更新时间不同），当前这两只的 08-06 净值东财与天天基金都还未披露，属正常现象。观察点：

- [ ] **披露时点**：100050 / 022184 的 08-06 净值何时披露（今晚 08-07 晚间 / 下周一 08-10）？QDII 因 T+1/T+2 延迟，最晚可能周一才出。
- [ ] **popup 自动切换**：东财一旦披露（`PDATE` 更新为 08-06），popup 应**无需任何手动操作**自动从 `-` 变为当日收益数字，且基金列次行日期自动变为「净值08-06」。
- [ ] **与天天基金一致性**：popup 显示的当日收益/涨跌幅应与天天基金更新后的口径一致（同源 `FundMNFInfo` 官方净值差）。
- [ ] **分组聚合自动计入**：当日收益从 `-` 变为数字后，分组「当日收益额」应自动把这两只计入（不再跳过）。
