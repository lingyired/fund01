...
pub fn normalize_fund(
    ...
    if is_hold {
        if let Some(a) = raw.get("allocations").and_then(|v| v.as_object()) {
        ...
        } else if let Some(prev_alloc) = prev.and_then(|p| {
            ...
            if old_shares > 0.0 {
                let groups: Vec<String> =
                    if let Some(g) = raw.get("groups").and_then(|v| v.as_array()) {
                        g.iter()
    ...
    Some(FundRecord {
        ...
        group: raw
            ...
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .or_else(|| prev.and_then(|p| p.group.clone())),
        groups: raw
            .get("groups")
            .and_then(|v| v.as_array())
            .map(|a| {
...
pub fn normalize_fund_map(
    source: &serde_json::Value,
    fallback_type: &str,
) -> HashMap<String, FundRecord> {
    ...
    for (key, raw) in obj {
        ...
        if let Some(o) = copy.as_object_mut() {
            o.insert(
                "code".to_string(),
                serde_json::Value::String(pad6(code_raw)),
            );
            o.insert(
                "type".to_string(),
...
pub fn normalize_config(payload: &serde_json::Value) -> AppConfig {
    ...
    if let Some(funds) = payload.get("funds").and_then(|v| v.as_object()) {
        for (key, raw) in funds {
            ...
            let t = if raw.get("type").and_then(|v| v.as_str()) == Some("hold") {
                "hold"
            } else {
                "watch"
            };
    ...
    if let Some(groups) = payload
        .pointer("/settings/holdingGroups")
        .and_then(|v| v.as_array())
    {
        for g in groups {
    ...
    if let Some(orders) = payload
        .pointer("/settings/holdingGroupOrders")
        .and_then(|v| v.as_object())
    {
        for (g, list) in orders {
            ...
            let cleaned: Vec<String> = list
                ...
                .filter_map(|c| {
                    ...
                    if code.len() == 6
                        && code.chars().all(|x| x.is_ascii_digit())
    ...
    if let Some(h) = settings_raw
        .and_then(|s| s.get("menubarHiddenGroups"))
        .and_then(|v| v.as_array())
    {
        for g in h {
            let key = g.as_str().unwrap_or("").trim().to_string();
    ...
    let menubar_layout: u8 = match settings_raw
        .and_then(|s| s.get("menubarLayout"))
        .and_then(|v| v.as_u64())
    {
        Some(2) => 2,
        _ => 0,
    ...
    let (d_top, d_bottom) = if menubar_layout == 2 {
        (9.0, 9.0)
    } else {
        (7.0, 11.0)
    };
    ...
    let menubar_top_font_size = clampf(
        settings_raw
            .and_then(|s| s.get("menubarTopFontSize"))
            .and_then(|v| v.as_f64()),
        d_top,
        7.0,
        10.0,
    ...
    let menubar_bottom_font_size = clampf(
        settings_raw
            .and_then(|s| s.get("menubarBottomFontSize"))
            .and_then(|v| v.as_f64()),
        d_bottom,
        10.0,
        14.0,
    ...
    let menubar_equal_font_size = clampf(
        settings_raw
            .and_then(|s| s.get("menubarEqualFontSize"))
            .and_then(|v| v.as_f64()),
        9.0,
        8.0,
        11.0,
    ...
    let group_tab_show_detail = settings_raw
        ...
        .unwrap_or(true);
    let group_tab_detail_mode =
        match settings_raw.and_then(|s| s.get("groupTabDetailMode").and_then(|v| v.as_str())) {
            Some("amount") => "amount".to_string(),
            _ => "percent".to_string(),
        };
    let quote_source =
        match settings_raw.and_then(|s| s.get("quoteSource").and_then(|v| v.as_str())) {
    ...
    let badge_mode = match settings_raw.and_then(|s| s.get("badgeMode").and_then(|v| v.as_str())) {
        Some("amount") | Some("hidden") => settings_raw
            .unwrap()
            .get("badgeMode")
            .unwrap()
            .as_str()
    ...
    let holdings_nav_position = if settings_raw
        .and_then(|s| s.get("holdingsNavPosition").and_then(|v| v.as_str()))
        == Some("side")
    {
        "side".to_string()
    } else {
    ...
    let theme = match settings_raw.and_then(|s| s.get("theme").and_then(|v| v.as_str())) {
        Some("light") | Some("dark") | Some("system") => settings_raw
            .unwrap()
            .get("theme")
            .unwrap()
            .as_str()
    ...
    let refresh_interval = clamp_refresh_interval(
        settings_raw
            ...
            .and_then(|v| {
                ...
                t.zip(n).map(|(t, n)| RefreshInterval {
                    trading: t as u64,
                    non_trading: n as u64,
                })
            })
            .as_ref(),
    );
